...
def build_relationship_graph(narrative_before: dict, extraction: dict) -> dict:
    ...
    gaps = detect_gaps(central_objects, relations, extraction)
...
def detect_gaps(central_objects: list[str], relations: list[dict], extraction: dict) -> list[dict]:
    gaps = []
    ...
    if prompt_like and has_output and not has_input_contract:
        gaps.append(
            {
